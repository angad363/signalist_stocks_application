...
export const getStocksDetails = async (symbol: string) => {
    ...
    try {
        ...
        const [quote, profile, financials] = await Promise.all([
            ...
            fetchJSON<QuoteData>(`${FINNHUB_BASE_URL}/quote?symbol=${cleanSymbol}&token=${token}`),
            // Company info - cache 1hr (rarely changes)
            fetchJSON<ProfileData>(`${FINNHUB_BASE_URL}/stock/profile2?symbol=${cleanSymbol}&token=${token}`, 3600),
            // Financial metrics (P/E, etc.) - cache 30min
            fetchJSON<FinancialsData>(`${FINNHUB_BASE_URL}/stock/metric?symbol=${cleanSymbol}&metric=all&token=${token}`, 1800),
        ]);
